...
func (sq *SubjectQuery) QueryLessonPlans() *LessonPlanQuery {
	query := (&LessonPlanClient{config: sq.config}).Query()
...
func (sq *SubjectQuery) WithLessonPlans(opts ...func(*LessonPlanQuery)) *SubjectQuery {
	query := (&LessonPlanClient{config: sq.config}).Query()
...
func (sq *SubjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subject, error) {
	...
	if query := sq.withLessonPlans; query != nil {
		if err := sq.loadLessonPlans(ctx, query, nodes,
			func(n *Subject) { n.Edges.LessonPlans = []*LessonPlan{} },
			func(n *Subject, e *LessonPlan) { n.Edges.LessonPlans = append(n.Edges.LessonPlans, e) }); err != nil {
			return nil, err
		}
	}
...
func (sq *SubjectQuery) loadLessonPlans(ctx context.Context, query *LessonPlanQuery, nodes []*Subject, init func(*Subject), assign func(*Subject, *LessonPlan)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(subject.LessonPlansTable)
		s.Join(joinT).On(s.C(lessonplan.FieldID), joinT.C(subject.LessonPlansPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(subject.LessonPlansPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(subject.LessonPlansPrimaryKey[1]))
