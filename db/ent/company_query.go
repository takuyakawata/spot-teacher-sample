...
func (cq *CompanyQuery) QueryLessonPlans() *LessonPlanQuery {
	query := (&LessonPlanClient{config: cq.config}).Query()
...
func (cq *CompanyQuery) QueryMembers() *UserQuery {
...
func (cq *CompanyQuery) Clone() *CompanyQuery {
	...
	return &CompanyQuery{
		...
		withLessonPlans: cq.withLessonPlans.Clone(),
...
func (cq *CompanyQuery) WithLessonPlans(opts ...func(*LessonPlanQuery)) *CompanyQuery {
	query := (&LessonPlanClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withLessonPlans = query
	return cq
}

// WithMembers tells the query-builder to eager-load the nodes that are connected to
// the "members" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CompanyQuery) WithMembers(opts ...func(*UserQuery)) *CompanyQuery {
	query := (&UserClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CompanyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Company, error) {
	...
	if query := cq.withLessonPlans; query != nil {
		if err := cq.loadLessonPlans(ctx, query, nodes,
			func(n *Company) { n.Edges.LessonPlans = []*LessonPlan{} },
			func(n *Company, e *LessonPlan) { n.Edges.LessonPlans = append(n.Edges.LessonPlans, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withMembers; query != nil {
		if err := cq.loadMembers(ctx, query, nodes,
			func(n *Company) { n.Edges.Members = []*User{} },
			func(n *Company, e *User) { n.Edges.Members = append(n.Edges.Members, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CompanyQuery) loadLessonPlans(ctx context.Context, query *LessonPlanQuery, nodes []*Company, init func(*Company), assign func(*Company, *LessonPlan)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(lessonplan.FieldCompanyID)
	}
	query.Where(predicate.LessonPlan(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(company.LessonPlansColumn), fks...))
	}))
...
func (cq *CompanyQuery) loadMembers(ctx context.Context, query *UserQuery, nodes []*Company, init func(*Company), assign func(*Company, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(company.MembersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
